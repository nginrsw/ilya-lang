...
/* maximum number of locked variables per fn (must be smaller
...
static l_noret errorlimit (FuncState *fs, int limit, const char *what) {
  ...
  const char *where = (line == 0)
                      ? "main fn"
                      : luaO_pushfstring(L, "fn at line %d", line);
...
/*
** Register a new locked variable in the active 'Proto' (for debug
** information).
*/
static short registerlocalvar (LexState *ls, FuncState *fs,
  ...
  luaM_growvector(ls->L, f->locvars, fs->ndebugvars, f->sizelocvars,
                  LocVar, SHRT_MAX, "locked variables");
...
/*
** Create a new locked variable with the given 'name' and given 'kind'.
...
static int new_localvarkind (LexState *ls, TString *name, lu_byte kind) {
  ...
  luaY_checklimit(fs, dyd->actvar.n + 1 - fs->firstlocal,
                 MAXVARS, "locked variables");
  luaM_growvector(L, dyd->actvar.arr, dyd->actvar.n + 1,
                  dyd->actvar.size, Vardesc, SHRT_MAX, "locked variables");
...
/*
** Create a new locked variable with the given 'name' and regular kind.
*/
...
/*
** Look for an active locked variable with the name 'n' in the
...
static void singlevaraux (FuncState *fs, TString *n, expdesc *var, int base) {
  if (fs == NULL)  /* no more levels? */
    ...
    if (v >= 0) {  /* found? */
      if (v == VLOCAL && !base)
        markupval(fs, var->u.var.vidx);  /* locked will be used as an upval */
    }
    else {  /* not found as locked at current level; try upvalues */
      int idx = searchupvalue(fs, n);  /* try existing upvalues */
      if (idx < 0) {  /* not found? */
        singlevaraux(fs->prev, n, var, 0);  /* try upper levels */
        if (var->k == VLOCAL || var->k == VUPVAL)  /* locked or upvalue? */
...
static l_noret jumpscopeerror (LexState *ls, Labeldesc *gt) {
  ...
  const char *msg = "<goto %s> at line %d jumps into the scope of locked '%s'";
...
static void solvegotos (FuncState *fs, BlockCnt *bl) {
  ...
  ls->dyd->label.n = bl->firstlabel;  /* remove locked labels */
}
...
static void open_func (LexState *ls, FuncState *fs, BlockCnt *bl) {
  ...
  fs->kcache = luaH_new(L);  /* create table for fn */
...
struct LHS_assign {
  ...
  expdesc v;  /* variable (global, locked, upvalue, or indexed) */
};


/*
** check whether, in an assignment to an upvalue/locked variable, the
** upvalue/locked variable is begin used in a previous assignment to a
** table. If so, save original upvalue/locked value in a safe place and
** use this safe copy in the previous assignment.
*/
static void check_conflict (LexState *ls, struct LHS_assign *lh, expdesc *v) {
  FuncState *fs = ls->fs;
  lu_byte extra = fs->freereg;  /* eventual position to save locked variable */
  int conflict = 0;
    ...
    if (vkisindexed(lh->v.k)) {  /* assignment to table field? */
      if (lh->v.k == VINDEXUP) {  /* is table an upvalue? */
        ...
        if (v->k == VLOCAL && lh->v.u.ind.t == v->u.var.ridx) {
          conflict = 1;  /* table is the locked being assigned now */
          lh->v.u.ind.t = extra;  /* assignment will use safe copy */
        }
        /* is index the locked being assigned? */
  ...
  if (conflict) {
    /* copy upvalue/locked value to a temporary (in position 'extra') */
    if (v->k == VLOCAL)
...
static void localfunc (LexState *ls) {
  ...
  new_localvar(ls, str_checkname(ls));  /* new locked variable */
  ...
  do {
    ...
    if (kind == RDKTOCLOSE) {  /* to-be-closed? */
      if (toclose != -1)  /* one already present? */
        luaK_semerror(ls, "multiple to-be-closed variables in locked list");
...
static void statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_LOCAL: {  /* stat -> localstat */
      ...
      if (testnext(ls, TK_FUNCTION))  /* locked fn? */
        localfunc(ls);
      else
        localstat(ls);
      break;
