...
static l_noret error_expected (LexState *ls, int token) {
  ilyaX_syntaxerror(ls,
      ilyaO_pushfstring(ls->L, "%s expected", ilyaX_token2str(ls, token)));
}
...
static l_noret errorlimit (FuncState *fs, int limit, const char *what) {
  ...
  const char *where = (line == 0)
                      ? "main fn"
                      : ilyaO_pushfstring(L, "fn at line %d", line);
  msg = ilyaO_pushfstring(L, "too many %s (limit is %d) in %s",
                             what, limit, where);
  ilyaX_syntaxerror(fs->ls, msg);
}


void ilyaY_checklimit (FuncState *fs, int v, int l, const char *what) {
...
#define check_condition(ls,c,msg)	{ if (!(c)) ilyaX_syntaxerror(ls, msg); }
...
static void check_match (LexState *ls, int what, int who, int where) {
  if (l_unlikely(!testnext(ls, what))) {
    if (where == ls->linenumber)  /* all in the same line? */
      ...
      ilyaX_syntaxerror(ls, ilyaO_pushfstring(ls->L,
             "%s expected (to close %s at line %d)",
              ilyaX_token2str(ls, what), ilyaX_token2str(ls, who), where));
    }
  }
}
...
static short registerlocalvar (LexState *ls, FuncState *fs,
  ...
  ilyaM_growvector(ls->L, f->locvars, fs->ndebugvars, f->sizelocvars,
                  LocVar, SHRT_MAX, "lock variables");
  while (oldsize < f->sizelocvars)
    f->locvars[oldsize++].varname = NULL;
  f->locvars[fs->ndebugvars].varname = varname;
  f->locvars[fs->ndebugvars].startpc = fs->pc;
  ilyaC_objbarrier(ls->L, f, varname);
...
static int new_localvarkind (LexState *ls, TString *name, lu_byte kind) {
  ...
  Dyndata *dyd = ls->dyd;
  Vardesc *var;
  ilyaY_checklimit(fs, dyd->actvar.n + 1 - fs->firstlocal,
                 MAXVARS, "lock variables");
  ilyaM_growvector(L, dyd->actvar.arr, dyd->actvar.n + 1,
...
#define new_localvarliteral(ls,v) \
    new_localvar(ls,  \
      ilyaX_newstring(ls, "" v, (sizeof(v)/sizeof(char)) - 1));
...
** fn.
*/
lu_byte ilyaY_nvarstack (FuncState *fs) {
...
static void check_readonly (LexState *ls, expdesc *e) {
  ...
  if (varname) {
    const char *msg = ilyaO_pushfstring(ls->L,
       "attempt to assign to const variable '%s'", getstr(varname));
    ilyaK_semerror(ls, msg);  /* error */
  }
}
...
static void adjustlocalvars (LexState *ls, int nvars) {
  FuncState *fs = ls->fs;
  int reglevel = ilyaY_nvarstack(fs);
  int i;
  for (i = 0; i < nvars; i++) {
    int vidx = fs->nactvar++;
...
static Upvaldesc *allocupvalue (FuncState *fs) {
  ...
  ilyaY_checklimit(fs, fs->nups + 1, MAXUPVAL, "upvalues");
  ilyaM_growvector(fs->ls->L, f->upvalues, fs->nups, f->sizeupvalues,
...
static int newupvalue (FuncState *fs, TString *name, expdesc *v) {
  ...
  up->name = name;
  ilyaC_objbarrier(fs->ls->L, fs->f, name);
  return fs->nups - 1;
}
...
static void singlevar (LexState *ls, expdesc *var) {
  ...
  if (var->k == VVOID) {  /* global name? */
    ...
    ilyaK_exp2anyregup(fs, var);  /* but could be a constant */
    codestring(&key, varname);  /* key is variable name */
    ilyaK_indexed(fs, var, &key);  /* env[varname] */
  }
}
...
static void adjust_assign (LexState *ls, int nvars, int nexps, expdesc *e) {
  ...
  if (hasmultret(e->k)) {  /* last expression has multiple returns? */
    ...
    if (extra < 0)
      extra = 0;
    ilyaK_setreturns(fs, e, extra);  /* last exp. provides the difference */
  }
  else {
    if (e->k != VVOID)  /* at least one expression? */
      ilyaK_exp2nextreg(fs, e);  /* close last expression */
    if (needed > 0)  /* missing values? */
      ilyaK_nil(fs, fs->freereg, needed);  /* complete with nils */
  }
  if (needed > 0)
    ilyaK_reserveregs(fs, needed);  /* registers for extra values */
  else  /* adding 'needed' is actually a subtraction */
    fs->freereg = cast_byte(fs->freereg + needed);  /* remove extra values */
}


#define enterlevel(ls)	ilyaE_incCstack(ls->L)
...
static l_noret jumpscopeerror (LexState *ls, Labeldesc *gt) {
  ...
  msg = ilyaO_pushfstring(ls->L, msg, getstr(gt->name), gt->line, varname);
  ilyaK_semerror(ls, msg);  /* raise the error */
}
...
static void closegoto (LexState *ls, int g, Labeldesc *label, int bup) {
  ...
  ilyaK_patchlist(ls->fs, gt->pc, label->pc);  /* goto jumps to label */
...
static int newlabelentry (LexState *ls, Labellist *l, TString *name,
                          int line, int pc) {
  int n = l->n;
  ilyaM_growvector(ls->L, l->arr, n, l->size,
...
static int newgotoentry (LexState *ls, TString *name, int line) {
  FuncState *fs = ls->fs;
  int pc = ilyaK_jump(fs);  /* create jump */
  ilyaK_codeABC(fs, OP_CLOSE, 0, 1, 0);  /* spaceholder, marked as dead */
...
static void createlabel (LexState *ls, TString *name, int line, int last) {
  ...
  int l = newlabelentry(ls, ll, name, line, ilyaK_getlabel(fs));
...
static void enterblock (FuncState *fs, BlockCnt *bl, lu_byte isloop) {
  ...
  bl->previous = fs->bl;
  fs->bl = bl;
  ilya_assert(fs->freereg == ilyaY_nvarstack(fs));
}
...
static l_noret undefgoto (LexState *ls, Labeldesc *gt) {
  ...
  msg = ilyaO_pushfstring(ls->L, msg, getstr(gt->name), gt->line);
  /* breaks are checked when created, cannot be undefined */
  ilya_assert(!eqstr(gt->name, ilyaS_newliteral(ls->L, "break")));
  ilyaK_semerror(ls, msg);
}
...
static Proto *addprototype (LexState *ls) {
  ...
  if (fs->np >= f->sizep) {
    int oldsize = f->sizep;
    ilyaM_growvector(L, f->p, fs->np, f->sizep, Proto *, MAXARG_Bx, "functions");
    while (oldsize < f->sizep)
      f->p[oldsize++] = NULL;
  }
  f->p[fs->np++] = clp = ilyaF_newproto(L);
  ilyaC_objbarrier(L, f, clp);
  return clp;
}
...
static void codeclosure (LexState *ls, expdesc *v) {
  ...
  init_exp(v, VRELOC, ilyaK_codeABx(fs, OP_CLOSURE, 0, fs->np - 1));
  ilyaK_exp2nextreg(fs, v);  /* fix it at the last register */
}
...
static void open_func (LexState *ls, FuncState *fs, BlockCnt *bl) {
  ...
  fs->bl = NULL;
  f->source = ls->source;
  ilyaC_objbarrier(L, f, f->source);
  f->maxstacksize = 2;  /* registers 0/1 are always valid */
  fs->kcache = ilyaH_new(L);  /* create table for fn */
...
static void close_func (LexState *ls) {
  ...
  FuncState *fs = ls->fs;
  Proto *f = fs->f;
  ilyaK_ret(fs, ilyaY_nvarstack(fs), 0);  /* final return */
  leaveblock(fs);
  ilya_assert(fs->bl == NULL);
  ilyaK_finish(fs);
  ilyaM_shrinkvector(L, f->code, f->sizecode, fs->pc, Instruction);
  ilyaM_shrinkvector(L, f->lineinfo, f->sizelineinfo, fs->pc, ls_byte);
  ilyaM_shrinkvector(L, f->abslineinfo, f->sizeabslineinfo,
                       fs->nabslineinfo, AbsLineInfo);
  ilyaM_shrinkvector(L, f->k, f->sizek, fs->nk, TValue);
  ilyaM_shrinkvector(L, f->p, f->sizep, fs->np, Proto *);
  ilyaM_shrinkvector(L, f->locvars, f->sizelocvars, fs->ndebugvars, LocVar);
  ilyaM_shrinkvector(L, f->upvalues, f->sizeupvalues, fs->nups, Upvaldesc);
  ls->fs = fs->prev;
...
static void fieldsel (LexState *ls, expdesc *v) {
  ...
  FuncState *fs = ls->fs;
  expdesc key;
  ilyaK_exp2anyregup(fs, v);
  ilyaX_next(ls);  /* skip the dot or colon */
  codename(ls, &key);
  ilyaK_indexed(fs, v, &key);
}


static void yindex (LexState *ls, expdesc *v) {
  /* index -> '[' expr ']' */
  ilyaX_next(ls);  /* skip the '[' */
  expr(ls, v);
  ilyaK_exp2val(ls->fs, v);
  checknext(ls, ']');
}
...
static void recfield (LexState *ls, ConsControl *cc) {
  ...
  if (ls->t.token == TK_NAME) {
    ilyaY_checklimit(fs, cc->nh, INT_MAX / 2, "items in a constructor");
    codename(ls, &key);
  }
  else  /* ls->t.token == '[' */
    yindex(ls, &key);
  ...
  checknext(ls, '=');
  tab = *cc->t;
  ilyaK_indexed(fs, &tab, &key);
  expr(ls, &val);
  ilyaK_storevar(fs, &tab, &val);
...
static void closelistfield (FuncState *fs, ConsControl *cc) {
  ...
  ilyaK_exp2nextreg(fs, &cc->v);
  cc->v.k = VVOID;
  if (cc->tostore >= cc->maxtostore) {
    ilyaK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);  /* flush */
    cc->na += cc->tostore;
...
static void lastlistfield (FuncState *fs, ConsControl *cc) {
  ...
  if (hasmultret(cc->v.k)) {
    ilyaK_setmultret(fs, &cc->v);
    ilyaK_setlist(fs, cc->t->u.info, cc->na, ILYA_MULTRET);
    cc->na--;  /* do not count last expression (unknown number of elements) */
  }
  else {
    if (cc->v.k != VVOID)
      ilyaK_exp2nextreg(fs, &cc->v);
    ilyaK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);
  }
  cc->na += cc->tostore;
}
...
static void field (LexState *ls, ConsControl *cc) {
  ...
  switch(ls->t.token) {
    case TK_NAME: {  /* may be 'listfield' or 'recfield' */
      if (ilyaX_lookahead(ls) != '=')  /* expression? */
        listfield(ls, cc);
      else
        recfield(ls, cc);
      break;
...
static void constructor (LexState *ls, expdesc *t) {
  ...
  int pc = ilyaK_codevABCk(fs, OP_NEWTABLE, 0, 0, 0, 0);
  ConsControl cc;
  ilyaK_code(fs, 0);  /* space for extra arg. */
  ...
  check_match(ls, /*{*/ '}', '{' /*}*/, line);
  lastlistfield(fs, &cc);
  ilyaK_settablesize(fs, pc, t->u.info, cc.na, cc.nh);
}
...
static void setvararg (FuncState *fs, int nparams) {
  ...
  ilyaK_codeABC(fs, OP_VARARGPREP, nparams, 0, 0);
}
...
static void parlist (LexState *ls) {
  ...
  if (ls->t.token != ')') {  /* is 'parlist' not empty? */
    do {
      switch (ls->t.token) {
        ...
        default: ilyaX_syntaxerror(ls, "<name> or '...' expected");
      }
  ...
  ilyaK_reserveregs(fs, fs->nactvar);  /* reserve registers for parameters */
}
...
static void funcargs (LexState *ls, expdesc *f) {
  ...
  switch (ls->t.token) {
    ...
    case TK_STRING: {  /* funcargs -> STRING */
      ...
      ilyaX_next(ls);  /* must use 'seminfo' before 'next' */
      break;
    }
    default: {
      ilyaX_syntaxerror(ls, "fn arguments expected");
    }
  }
  ...
  if (hasmultret(args.k))
    ...
    if (args.k != VVOID)
      ilyaK_exp2nextreg(fs, &args);  /* close last argument */
    nparams = fs->freereg - (base+1);
  }
  init_exp(f, VCALL, ilyaK_codeABC(fs, OP_CALL, base, nparams+1, 2));
...
static void primaryexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    default: {
      ilyaX_syntaxerror(ls, "unexpected symbol");
    }
  }
}
...
static void simpleexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    case TK_DOTS: {  /* vararg */
      ...
      init_exp(v, VVARARG, ilyaK_codeABC(fs, OP_VARARG, 0, 0, 1));
      break;
    }
    case '{' /*}*/: {  /* constructor */
      constructor(ls, v);
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  if (uop != OPR_NOUNOPR) {  /* prefix (unary) operator? */
    int line = ls->linenumber;
    ilyaX_next(ls);  /* skip operator */
    subexpr(ls, v, UNARY_PRIORITY);
    ilyaK_prefix(ls->fs, uop, v, line);
  }
  else simpleexp(ls, v);
  ...
  while (op != OPR_NOBINOPR && priority[op].left > limit) {
    ...
    int line = ls->linenumber;
    ilyaX_next(ls);  /* skip operator */
    ilyaK_infix(ls->fs, op, v);
    /* read sub-expression with higher priority */
    nextop = subexpr(ls, &v2, priority[op].right);
    ilyaK_posfix(ls->fs, op, v, &v2, line);
    op = nextop;
  }
  leavelevel(ls);
...
static void check_conflict (LexState *ls, struct LHS_assign *lh, expdesc *v) {
  ...
  if (conflict) {
    ...
    if (v->k == VLOCAL)
      ilyaK_codeABC(fs, OP_MOVE, extra, v->u.var.ridx, 0);
    else
      ilyaK_codeABC(fs, OP_GETUPVAL, extra, v->u.info, 0);
    ilyaK_reserveregs(fs, 1);
  }
}
...
static void restassign (LexState *ls, struct LHS_assign *lh, int nvars) {
  ...
  if (testnext(ls, ',')) {  /* restassign -> ',' suffixedexp restassign */
    ...
    if (nexps != nvars)
      ...
      ilyaK_setoneret(ls->fs, &e);  /* close last expression */
      ilyaK_storevar(ls->fs, &lh->v, &e);
...
static void breakstat (LexState *ls, int line) {
  ...
  ilyaX_syntaxerror(ls, "break outside loop");
 ok:
  bl->isloop = 2;  /* signal that block has pending breaks */
  ilyaX_next(ls);  /* skip break */
  newgotoentry(ls, ilyaS_newliteral(ls->L, "break"), line);
}
...
static void checkrepeated (LexState *ls, TString *name) {
  ...
  if (l_unlikely(lb != NULL)) {  /* already defined? */
    ...
    msg = ilyaO_pushfstring(ls->L, msg, getstr(name), lb->line);
    ilyaK_semerror(ls, msg);  /* error */
  }
}
...
static void whilestat (LexState *ls, int line) {
  ...
  int condexit;
  BlockCnt bl;
  ilyaX_next(ls);  /* skip WHILE */
  whileinit = ilyaK_getlabel(fs);
  condexit = cond(ls);
  enterblock(fs, &bl, 1);
  checknext(ls, TK_DO);
  block(ls);
  ilyaK_jumpto(fs, whileinit);
  check_match(ls, TK_END, TK_WHILE, line);
  leaveblock(fs);
  ilyaK_patchtohere(fs, condexit);  /* false conditions finish the loop */
}
...
static void repeatstat (LexState *ls, int line) {
  ...
  int condexit;
  FuncState *fs = ls->fs;
  int repeat_init = ilyaK_getlabel(fs);
  BlockCnt bl1, bl2;
  enterblock(fs, &bl1, 1);  /* loop block */
  enterblock(fs, &bl2, 0);  /* scope block */
  ilyaX_next(ls);  /* skip REPEAT */
  statlist(ls);
  check_match(ls, TK_UNTIL, TK_REPEAT, line);
  condexit = cond(ls);  /* read condition (inside scope block) */
  leaveblock(fs);  /* finish scope */
  if (bl2.upval) {  /* upvalues? */
    int exit = ilyaK_jump(fs);  /* normal exit must jump over fix */
    ilyaK_patchtohere(fs, condexit);  /* repetition must close upvalues */
    ilyaK_codeABC(fs, OP_CLOSE, reglevel(fs, bl2.nactvar), 0, 0);
    condexit = ilyaK_jump(fs);  /* repeat after closing upvalues */
    ilyaK_patchtohere(fs, exit);  /* normal exit comes to here */
  }
  ilyaK_patchlist(fs, condexit, repeat_init);  /* close the loop */
...
static void forbody (LexState *ls, int base, int line, int nvars, int isgen) {
  ...
  int prep, endfor;
  checknext(ls, TK_DO);
  prep = ilyaK_codeABx(fs, forprep[isgen], base, 0);
  fs->freereg--;  /* both 'forprep' remove one register from the stack */
  enterblock(fs, &bl, 0);  /* scope for declared variables */
  adjustlocalvars(ls, nvars);
  ilyaK_reserveregs(fs, nvars);
  block(ls);
  leaveblock(fs);  /* end of scope for declared variables */
  fixforjump(fs, prep, ilyaK_getlabel(fs), 0);
  if (isgen) {  /* generic for? */
    ilyaK_codeABC(fs, OP_TFORCALL, base, 0, nvars);
    ilyaK_fixline(fs, line);
  }
  endfor = ilyaK_codeABx(fs, forloop[isgen], base, 0);
...
static void forlist (LexState *ls, TString *indexname) {
  ...
  ilyaK_checkstack(fs, 2);  /* extra space to call iterator */
...
static void forstat (LexState *ls, int line) {
  ...
  ilyaX_next(ls);  /* skip 'for' */
  varname = str_checkname(ls);  /* first variable name */
  switch (ls->t.token) {
    case '=': fornum(ls, varname, line); break;
    case ',': case TK_IN: forlist(ls, varname); break;
    default: ilyaX_syntaxerror(ls, "'=' or 'in' expected");
  }
...
static void test_then_block (LexState *ls, int *escapelist) {
  ...
  FuncState *fs = ls->fs;
  int condtrue;
  ilyaX_next(ls);  /* skip IF or ELSEIF */
  condtrue = cond(ls);  /* read condition */
  checknext(ls, TK_THEN);
  block(ls);  /* 'then' part */
  if (ls->t.token == TK_ELSE ||
      ls->t.token == TK_ELSEIF)  /* followed by 'else'/'elseif'? */
    ilyaK_concat(fs, escapelist, ilyaK_jump(fs));  /* must jump over it */
  ilyaK_patchtohere(fs, condtrue);
}
...
static void ifstat (LexState *ls, int line) {
  ...
  ilyaK_patchtohere(fs, escapelist);  /* patch escape list to 'if' end */
}
...
static lu_byte getlocalattribute (LexState *ls) {
  ...
  if (testnext(ls, '<')) {
    ...
    if (strcmp(attr, "const") == 0)
      ...
      ilyaK_semerror(ls,
        ilyaO_pushfstring(ls->L, "unknown attribute '%s'", attr));
  }
...
static void checktoclose (FuncState *fs, int level) {
  if (level != -1) {  /* is there a to-be-closed variable? */
    marktobeclosed(fs);
    ilyaK_codeABC(fs, OP_TBC, reglevel(fs, level), 0, 0);
  }
}
...
static void localstat (LexState *ls) {
  ...
  do {
    ...
    if (kind == RDKTOCLOSE) {  /* to-be-closed? */
      if (toclose != -1)  /* one already present? */
        ilyaK_semerror(ls, "multiple to-be-closed variables in lock list");
  ...
  if (nvars == nexps &&  /* no adjustments? */
      ...
      ilyaK_exp2const(fs, &e, &var->k)) {  /* compile-time constant? */
...
static void funcstat (LexState *ls, int line) {
  ...
  int ismethod;
  expdesc v, b;
  ilyaX_next(ls);  /* skip FUNCTION */
  ismethod = funcname(ls, &v);
  body(ls, &b, ismethod, line);
  check_readonly(ls, &v);
  ilyaK_storevar(ls->fs, &v, &b);
  ilyaK_fixline(ls->fs, line);  /* definition "happens" in the first line */
}
...
static void retstat (LexState *ls) {
  ...
  int first = ilyaY_nvarstack(fs);  /* first slot to be returned */
  if (block_follow(ls, 1) || ls->t.token == ';')
    nret = 0;  /* return no values */
  else {
    nret = explist(ls, &e);  /* optional return values */
    if (hasmultret(e.k)) {
      ilyaK_setmultret(fs, &e);
      if (e.k == VCALL && nret == 1 && !fs->bl->insidetbc) {  /* tail call? */
        SET_OPCODE(getinstruction(fs,&e), OP_TAILCALL);
        ilya_assert(GETARG_A(getinstruction(fs,&e)) == ilyaY_nvarstack(fs));
      }
      nret = ILYA_MULTRET;  /* return all values */
    }
    else {
      if (nret == 1)  /* only one single value? */
        first = ilyaK_exp2anyreg(fs, &e);  /* can use original slot */
      else {  /* values must go to the top of the stack */
        ilyaK_exp2nextreg(fs, &e);
        ilya_assert(nret == fs->freereg - first);
      }
    }
  }
  ilyaK_ret(fs, first, nret);
...
static void statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_DBCOLON: {  /* stat -> label */
      ilyaX_next(ls);  /* skip double colon */
      labelstat(ls, str_checkname(ls), line);
      break;
    }
    case TK_RETURN: {  /* stat -> retstat */
      ilyaX_next(ls);  /* skip RETURN */
      retstat(ls);
      break;
    }
  ...
  ilya_assert(ls->fs->f->maxstacksize >= ls->fs->freereg &&
             ls->fs->freereg >= ilyaY_nvarstack(ls->fs));
  ls->fs->freereg = ilyaY_nvarstack(ls->fs);  /* free registers */
  leavelevel(ls);
}
...
static void mainfunc (LexState *ls, FuncState *fs) {
  ...
  env->kind = VDKREG;
  env->name = ls->envn;
  ilyaC_objbarrier(ls->L, fs->f, env->name);
  ilyaX_next(ls);  /* read first token */
...
LClosure *ilyaY_parser (ilya_State *L, ZIO *z, Mbuffer *buff,
                       Dyndata *dyd, const char *name, int firstchar) {
  LexState lexstate;
  FuncState funcstate;
  LClosure *cl = ilyaF_newLclosure(L, 1);  /* create main closure */
  setclLvalue2s(L, L->top.p, cl);  /* anchor it (to avoid being collected) */
  ilyaD_inctop(L);
  lexstate.h = ilyaH_new(L);  /* create table for scanner */
  sethvalue2s(L, L->top.p, lexstate.h);  /* anchor it */
  ilyaD_inctop(L);
  funcstate.f = cl->p = ilyaF_newproto(L);
  ilyaC_objbarrier(L, cl, cl->p);
  funcstate.f->source = ilyaS_new(L, name);  /* create and anchor TString */
  ilyaC_objbarrier(L, funcstate.f, funcstate.f->source);
  lexstate.buff = buff;
  lexstate.dyd = dyd;
  dyd->actvar.n = dyd->gt.n = dyd->label.n = 0;
  ilyaX_setinput(L, &lexstate, z, funcstate.f->source, firstchar);
